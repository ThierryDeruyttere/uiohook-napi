...
/* Refreshes the cached keyboard layout data used by keycode_to_unicode.
 * MUST be called on the main runloop (the TIS calls are not thread-safe off
 * main). The hook thread translates from the cache so the event tap callback
 * never blocks on the host app's main queue.
 */
extern void refresh_keyboard_layout_cache();
#endif
